...
import { useState, useEffect } from "react";
import { useNavigate } from "react-router-dom";
// Mengubah import absolute (@/) menjadi relative (../../) untuk menghindari error alias
import { supabase } from "../../lib/SupabaseClient";
...
export default function OnboardingPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Kita menyesuaikan payload dengan Schema Table 'profiles' yang Anda berikan.
    // Field yang tidak ada di database dikomentari agar tidak error.
    const { error } = await supabase.from("profiles").upsert({
      id: user.id,
      name,
      student_id: studentId,
      email: email, // Ada di schema
      phone,
      major,
      campus,
      bio,
      // skills: [], // Ada di schema (text[]), tapi belum ada inputnya di form, bisa dikosongkan atau diisi array kosong
      
      // Field di bawah ini ADA di Form tapi TIDAK ADA di Table Database Anda.
      // Saya komen agar tidak error saat save.
      // secondary_email: secondaryEmail || null,
      // semester: semester,
      // location: location,
      // languages: languages.split(",").map((lang) => lang.trim()).filter(Boolean),
      ...
      <div className="bg-white p-8 rounded-2xl shadow-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="space-y-4">
            ...
            {/* Input ini tetap ditampilkan di UI, tapi datanya tidak akan tersimpan ke DB karena kolomnya belum ada */}
            <input
              type="email"
              placeholder="Secondary Email (optional) - Not saved yet"
              value={secondaryEmail}
              onChange={(e) => setSecondaryEmail(e.target.value)}
              className="border rounded-lg p-2.5 w-full focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            />
          ...
          <div className="space-y-4 pt-4 border-t">
            ...
            <input
              type="text"
              placeholder="Current Semester (e.g., Semester 3) - Not saved yet"
              value={semester}
              onChange={(e) => setSemester(e.target.value)}
              className="border rounded-lg p-2.5 w-full focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            />
          ...
          <div className="space-y-4 pt-4 border-t">
            ...
            <input
              type="text"
              placeholder="City / Location - Not saved yet"
              value={location}
              onChange={(e) => setLocation(e.target.value)}
              className="border rounded-lg p-2.5 w-full focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            />
            ...
            <input
              type="text"
              placeholder="Languages Spoken - Not saved yet"
              value={languages}
              onChange={(e) => setLanguages(e.target.value)}
              className="border rounded-lg p-2.5 w-full focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            />
